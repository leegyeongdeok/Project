            ...
            <div class="content" >
		     <h1 class="business_post_brd_title">
		    	...
		    	<span class="business_post_detail_tbl_cell" id="businessPostDetailTitle">
		    		${ownerBoardDetail.title}
		    	</span>
		    
		    	
		    	<span class="business_post_brd_date"><tf:formatDateTime value="${ownerBoardDetail.registeredAt}" pattern="yyyy.MM.dd"/></span>
		    </h1>
		    <div class="business_post_brd_editor">
		<span><p class="business_post_brd_writer" style="float: left; " id="businessPostDetailWriter">작성자: ${ownerBoardDetail.ownerUser.name}</p> <p class="business_post_brd_nik" id="businessPostDetailNikname"></p></span>
		        <div class="business_post_editor_area">
                    ${ownerBoardDetail.content}
		        </div>
		       ...
		       <!--  뎃글 -->
                <section class="business_post_ReviewList">
                            ...
                            <span class="business_post_ReviewList__business_post_SuffixMobile">
                                <span>(
                                     <span class="business_post_ReviewList__AllCount" id="businessPostDetailAllCount">
                                         ${ownerBoardDetail.ownerBoardReplyList.size()}
                                     </span>
                                )</span>
                        </h2>
                    </header>
                    <fieldset>
                         <span class="search_bar">
                             <form id="insertBoardFrm">
                                 ...
                                 <input name="owner_board_no" id="ownerBoardNo" type="hidden" value="${ownerBoardDetail.ownerBoardNo}" >
                                 <input name="owner_id" id="ownerId" type="hidden">
                             </form>
                         </span>
                    ...
                    <ul class="business_post_ReviewList__ReviewList">
                        <c:forEach var="reply" items="${ownerBoardDetail.ownerBoardReplyList}">
                            <li class="business_post_ReviewItem business_post_ReviewList__ReviewItem">
                                <div class="business_post_ReviewItem__business">
                                    <div class="business_post_ReviewItem__businessPictureWrap">
                                        <span class="business_post_ReviewItem__businessname" id="businessInquiryDetailUserNickName">${reply.ownerUser.name}</span>
		    ...
		    <table class="business_post_tbl_pren">
		        ...
		        <tbody>
                <c:if test="${!empty beforeOwnerBoard}">
                    <tr>
                        <th scope="row">이전 글</th>
                        <td>
                            <a onclick="switchBoard(${beforeOwnerBoard.ownerBoardNo})" class="title_link">${beforeOwnerBoard.title}</a>
                        </td>
                    </tr>
                </c:if>

                <c:if test="${!empty afterOwnerBoard}">
                    <tr>
                        <th scope="row">다음 글</th>
                        <td>
                            <a onclick="switchBoard(${afterOwnerBoard.ownerBoardNo})" class="title_link">${afterOwnerBoard.title}</a>
                        </td>
                    </tr>
                </c:if>
		        </tbody>
