      ...
      <div class="business_post_listtable_search">
            <span class="selectbox black">
                <select id="type_selecter" name="option">
                  ...
                  <option value="name" <c:if test="${stype eq 'name'}"> selected</c:if> >닉네임</option>
            </select>
   ...
   <div class="business_post_listtable">
      <div style="height: 40px;">
         <p class="business_post_listtbl_count">
            총 <strong class="business_post_list_count" id="businessPostListCount">${generalBoard.number}</strong>건
         </p>
      </div>
      ...
      <table class="business_post_listtbl_board" id="businessPostListBoard">
         ...
         <tbody>
         <c:forEach var="board" items="${generalBoard.content}">
            <tr>
               <input id="${board.generalBoardNo}" type="hidden" value="${board.generalBoardNo}">
               <td id="notice_num">${board.generalBoardNo}</td>
               <td id="notice_name">${board.generalUser.nickname}</td>
               <td><a onclick="detail(${board.generalBoardNo})" id="notice_title" class="business_noticetitle_link" onclick="">${board.title}</a></td>
...
<script>
   ...
   var nowPage = <%=pages%>;
   var totalP = ${generalBoard.totalPages};
   var type = $("#type").val();
   ...
   function pageUp() {
      if(nowPage != totalP){
         nowPage++;
         location.href="Board?id="+account+"&name="+userName+"&type=general"+"&page="+nowPage;
      }else{
         alert("마지막 페이지입니다.");
      }
   ...
   function pageBack() {
      if(nowPage == totalP){
         ...
         nowPage = ${generalBoard.totalPages};
         location.href="Board?id="+account+"&name="+userName+"&type=general"+"&page="+nowPage;
      }
   ...
   function pageSelect(page) {
      nowPage = page;
      location.href="Board?id="+account+"&name="+userName+"&type=general"+"&page="+page;
   }

   function searchBoard(search) {
      var selectType = $("#type_selecter option:selected").val();
      location.href="Board?id="+account+"&name="+userName+"&type=general"+"&search="+search+"&stype="+selectType;
   }

   function writing() {
      location.href="BoardWriting?id="+account+"&name="+userName+"&type=general";
   }
</script>
</body>
</html>